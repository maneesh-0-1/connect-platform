...
export default function DelegateLogin() {
    ...
    const handleVerifyOTP = async (e: React.FormEvent) => {
        ...
        if (res.ok) {
            ...
            // Successful login
            router.push(`/delegate/edit`); // Go to edit dashboard (or profile)
        } else {
            alert('Invalid Code');
        }
        setLoading(false);
        ...
        <div className="min-h-screen flex items-center justify-center p-4 bg-[url('/grid-bg.svg')] bg-cover relative">
            ...
            <Card className="w-full max-w-md relative z-10 glass-card">
                <div className="text-center mb-8">
                    <h1 className="text-2xl font-bold neon-text mb-2">Delegate Login</h1>
                    <p className="text-gray-400 text-sm">Access your digital card</p>
                </div>
                ...
                {step === 'EMAIL' ? (
                    ...
                    <form onSubmit={handleVerifyOTP} className="space-y-6">
                        ...
                        <Input
                            ...
                            required
                        />
                        <Button loading={loading} className="w-full">Verify & Access</Button>
                    </form>
                )}
            </Card>
        </div>
